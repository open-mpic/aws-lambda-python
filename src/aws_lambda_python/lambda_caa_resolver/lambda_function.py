...
def does_value_list_permit_issuance(value_list, caa_domains):
...
def lambda_handler(event, context):
    ...
    if has_unknown_critical_flags:
        ...
        if is_wc_domain and len(issue_wild_tags) > 0:
            valid_for_issue = does_value_list_permit_issuance(issue_wild_tags, caa_domains)
        elif len(issue_tags) > 0:
            valid_for_issue = does_value_list_permit_issuance(issue_tags, caa_domains)
        else:
